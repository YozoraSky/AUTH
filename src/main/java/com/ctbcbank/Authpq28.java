...
import com.ctbcbank.properties.Authpq28Properties;
...
public class Authpq28 extends Auth{
	...
	@Autowired
	Authpq28Properties authpq28Properties;
	...
	public void start() {
		...
		try {
			...
			List<Map<String,Object>> list = namedParameterJdbcTemplate.queryForList(authpq28Properties.getCheckworkdaySql(), params);
			boolean done = false;
			for(Map<String, Object> map : list) {
//				Authpq28屬於類別2
				if((Short)map.get("FileType") == 2) {
					done = true;
//					mode = 1 當日須轉入, mode = 0 當日不須轉入
					if((Short)map.get("Mode") == 1) {
						logger.info("工作日---Authpq28授權批次");
						execute(ftp, authpq28Properties.getRemotePath(), authpq28Properties.getDownloadPath(), authpq28Properties.getBackupPath(), authProperties.getDeCompressKey());
						break;
					}
					else {
						logger.info("休假日");
...
//			若當天找不到類別2，則按照預設日去執行(星期二~星期六要執行，日一不用)
			if(!done) {
				done = true;
				if(!getWeekDays(today).equals("星期日") && !getWeekDays(today).equals("星期一")) {
					logger.info("未找到類別2，按照預設 (星期二~星期六) 日期執行Authpq28授權批次");
					execute(ftp, authpq28Properties.getRemotePath(), authpq28Properties.getDownloadPath(), authpq28Properties.getBackupPath(), authProperties.getDeCompressKey());
				}
				else {
					logger.info("未找到類別2，且不屬於預設 (星期二~星期六) 日期");
				}
			}
		} catch (Exception e) {
...
//			交易類別
			...
			while (batchArgsForInsert.size() >= 1000) {
				namedParameterJdbcTemplate.batchUpdate(authpq28Properties.getInsertSql(),batchArgsForInsert.toArray(new MapSqlParameterSource[0]));
		...
		logger.info("Authpq28 insert count : " + insertCount);
		params.put("FilePath", getFileName());
		params.put("SuccessCount", insertCount);
		params.put("FailCount", "0");
		namedParameterJdbcTemplate.update(authpq28Properties.getStatusSql(), params);
	}
