...
public class Authpq28 extends Auth {
	...
	public void checkForExecution(Date date) throws Exception {
		...
		List<Map<String, Object>> list = namedParameterJdbcTemplate
				.queryForList(authpq28Properties.getCheckworkdaySql(), params);
		boolean done = false;
...
//					mode = 1 當日須轉入, mode = 0 當日不須轉入
				if ((Short) map.get("Mode") == 1) {
					...
					execute(ftp, authpq28Properties.getRemotePath(), authpq28Properties.getDownloadPath(),
							authpq28Properties.getBackupPath(), authProperties.getDeCompressKey());
					break;
				} else {
					logger.info("休假日");
					break;
...
//			交易類別
			...
			while (batchArgsForInsert.size() >= 1000) {
				namedParameterJdbcTemplate.batchUpdate(authpq28Properties.getInsertSql(),
	...
	public void setFileName(Date date) {
		...
		this.fileName = authpq28Properties.getName() + sdf.format(date);
	}
}
