...
public class Authpq27 extends Auth{
	...
	public void start() {
		...
		try {
			...
			List<Map<String,Object>> list = namedParameterJdbcTemplate.queryForList(authpq27Properties.getCheckworkdaySql(), params);
			boolean done = false;
...
//			銀行名稱
			...
			while(batchArgsForInsert.size()>=1000) {
				namedParameterJdbcTemplate.batchUpdate(authpq27Properties.getInsertSql(), batchArgsForInsert.toArray(new MapSqlParameterSource[0]));
	...
	public String getFileName() {
		...
		String fileName = authpq27Properties.getName() + sdf.format(new Date(time));
		return fileName;
	}
}
